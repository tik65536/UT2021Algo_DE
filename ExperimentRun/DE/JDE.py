...
class JDEV2_MLP():
    ...
    def fit(self,config,id_,p=None):
        ...
        for e in range(self.epoch):
            ...
            print(f'JDE ConfigID: {id_:3d}, Epoch: {e:3d}, Training Loss: {(batchloss/batch):10.8f}, Validation Loss: {(vloss):10.8f},Best: {best:10.8f}, Accuracy: {accuracy}, StopCount/Limit: {stop:3d}/{self.stopcount:3d}, Time:{(end-start):10.8f}')
    ...
    def run(self,beta=0.5,cr=0.9):
        ...
        bestGen = 0
        print(f'JDE Init Run Best: {currentbest}, Mean: {currentmean}, ID:{currentbestidx}, config: {current_gen[currentbestidx]}')
        #Generation Run
        for i in range(self.maxiter):
            ...
            if(currentbest<overallBest):
                ...
                overallBestAccuracy = accuracy[currentbestidx]
                bestGen = i
            print(f'JDE Run {i:3d} CurrentBest: {currentbest:10.8f}, Mean: {currentmean:10.8f}, OverallBest: {overallBest:10.8f}/{bestGen:3d}, config: {current_gen[currentbestidx]}, updatecount: {updatecount:3d}, Generation RunTime: {(end-start):10.8f}')
        ...
        print(f'JDE Run Completed : Best Score: {overallBest} , Config: {overallBestConfig}, find in Gen: {bestGen}')
        return
